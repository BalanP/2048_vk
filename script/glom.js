...
KeyboardInputManager.prototype.listen = function () {
  ...
  var t = document.querySelector(".brag");
  t.addEventListener("click", brag);
  t.addEventListener("touchend", brag);
...
HTMLActuator.prototype.addTile = function (tile) {
  ...
  $('.tile.tile-' + tile.value).inner = names[Math.log(tile.value)/Math.LN2]+' (' + tile.value + ')';
...
HTMLActuator.prototype.message = function (won) {
    ...
    var message = won ? "Ты победил" + mysex + "!" : "Ты проиграл" + mysex + "!";
...
window.requestAnimationFrame(function () {
  VK.init(function() { 
     VK.api('friends.get',{order : 'random', count : '11', fields : ['photo_100']},function(friends) {
     	if (friends.response) {
      ...
      new GameManager(4, KeyboardInputManager, HTMLActuator, LocalScoreManager);
     		...
     		alert(friends.error.error_msg);
     	}
     	
  VK.api('users.get', {fields : "sex", test_mode : 1}, function(response) {
    if (response.hasOwnProperty("error")) {
     ...
     alert("Не получилось соединиться с интерфейсом VK. Это очень плохо, и вы не должны этого видеть");
}, '5.21');
});
